...
module.exports = {
	...
	add_movie: function(id, size, path, seen) {
		manager.connect().then(function(db) {
			const collection = db.collection('movies');
			collection.insertOne({id: id, size: size, path: path, lastseen: seen}, function(err, result) {
			});
		})
	},
	add_episode: function(id, size, path, seen) {
		manager.connect().then(function(db) {
			const collection = db.collection('episodes');
			collection.insertOne({id: id, size: size, path: path, lastseen: seen}, function(err, result) {
			});
